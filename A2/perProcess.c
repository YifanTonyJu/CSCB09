...
void print_processFD_table(ProcessInfo* complete_pids_arr, pid_t pid) {
    ...
    if (pid >= 1) {
        ...
        printf(PERPROCESS_TABLE_HEADER);
        ...
        while(current != NULL) {
            printf(PERPROCESS_TABLE_INFOROW_COL1_FOR_SINGLE_PID, current->pid_ptr->pid);
            printf(PERPROCESS_TABLE_INFOROW_COL2, current->fd);
        ...
        for(pid_index = 0; pid_index < complete_pids_arr->pid_num; ++pid_index) {
            ...
            while(current != NULL) {
                printf(PERPROCESS_TABLE_INFOROW_ROWINDEX, table_row_index);
                printf(PERPROCESS_TABLE_INFOROW_COL1_FOR_COMPLETE_PID, current->pid_ptr->pid);
                printf(PERPROCESS_TABLE_INFOROW_COL2, current->fd);
                table_row_index += 1;
                current = current->next;
            }
